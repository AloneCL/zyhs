    ...
    (function ($, window) {
        ...
        function initWebUpload(item, options) {
            ...
            var webuploaderoptions = $.extend({
                    ...
                    // 文件接收服务端。
                    server: 'uploader.action?packid=' + $packid,
            ...
            if (fileDataStr && opts.PostbackHold) {
                ...
                $.each(jsonData.fileList, function (index, fileData) {
                    ...
                    $list.append('<div id="' + newid + '" class="item" style="clear:both;">' +
            ...
            if (opts.auto) {
                ...
                uploader.on('fileQueued', function (file) {
                    $list.append('<div id="' + $(item)[0].id + file.id + '" class="item">' +
                        '<span class="glyphicon glyphicon-remove webuploadDelbtn" aria-hidden="true"></span>' +
                        '<span class="webuploadinfo">' + file.name + '</span>' +
                        '<span class="webuploadstate">正在上传...</span><br />' +
                        '</div>');
                    uploader.upload();
                });
            } else {
                uploader.on('fileQueued', function (file) {//队列事件
                    $list.append('<div id="' + $(item)[0].id + file.id + '" class="item">' +
                        '<span class="glyphicon glyphicon-remove webuploadDelbtn" aria-hidden="true"></span>' +
                        '<span class="webuploadinfo">' + file.name + '</span>' +
                        '<span class="webuploadstate">等待上传...</span><br />' +
